...
dotnet clean "$TOOLS_PRJ" -c Release -f net8.0 -r osx-arm64 -v quiet --nologo

# Publish executable for macOS
dotnet publish "$TOOLS_PRJ" -c Release -f net8.0 -o "$TOOLS_DIR/FlutterSync" -p:PublishSingleFile=true -r osx-arm64 --no-self-contained -v minimal --nologo